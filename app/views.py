...
def os_confirm(request):
    ...
    try:
        ...
        if c.status == 'new':
            ...
            if otp == nowotp:
                url = (
                        'http://210.107.78.156:8001/add_OS/' + contract_id + '- Exporter: ' + owner + '- importer: ' + share_user + '- offer sheet: ' + hash)
...
def lcr_confirm(request):
    ...
    try:
        ...
        if c.status == 'new':
            ...
            if otp == nowotp:
                url = (
                        'http://210.107.78.156:8001/add_LCR/' + contract_id + '- importer: ' + owner + '- bank: ' + share_user + '- letter of credit request: ' + hash)
                response = requests.post(url)
                res = response.text
                if res == "Fail":
                    result_dict['result'] = 'Fail'
...
def lc_confirm(request):
    ...
    try:
        ...
        if c.status == 'new':
            ...
            if otp == nowotp:
                url = (
                        'http://210.107.78.156:8001/add_LC/' + contract_id + '- bank: ' + owner + '- exporter: ' + share_user + '- letter of credit: ' + hash)
                response = requests.post(url)
                res = response.text
                if res == "Fail":
                    result_dict['result'] = 'Fail'
...
def sr_confirm(request):
    ...
    try:
        ...
        if c.status == 'new':
            ...
            if otp == nowotp:
                url = (
                        'http://210.107.78.156:8001/add_SR/' + contract_id + '- exporter: ' + owner + '- shipper: ' + share_user + '- shipping request: ' + hash)
                response = requests.post(url)
                res = response.text
                if res == "Fail":
                    result_dict['result'] = 'Fail'
...
def bl_confirm2(request):
    ...
    try:
        ...
        if c.status3 == 'new':
            ...
            if otp == nowotp:
                url = (
                        'http://210.107.78.156:8001/add_BL/' + contract_id + '- shipper: ' + owner + '- exporter: ' + share_user + '- bills of letter: ' + hash)
                response = requests.post(url)
                res = response.text
                if res == "Fail":
                    result_dict['result'] = 'Fail'
...
def ci_confirm(request):
    ...
    try:
        ...
        if c.status == 'new':
            ...
            if otp == nowotp:
                url = (
                        'http://210.107.78.156:8001/add_CI/' + contract_id + '- exporter: ' + owner + '- importer: ' + share_user + '- commercial invoice: ' + hash)
                response = requests.post(url)
                res = response.text
                if res == "Fail":
                    result_dict['result'] = 'Fail'
...
def do_confirm(request):
    ...
    try:
        ...
        if c.status == 'new':
            ...
            if otp == nowotp:
                url = (
                            'http://210.107.78.156:8001/add_DO/' + contract_id + '- shipper: ' + owner + '- importer: ' + share_user + '- delivery order: ' + hash)
                response = requests.post(url)
                res = response.text
                if res == "Fail":
                    result_dict['result'] = 'Fail'
...
def checkcontract(request):
    try:
        ...
        url = ("http://210.107.78.156:8001/keyHistory/%s" % cid)
...
def osdetail(request):
    return render(request, 'app/osdetail.html', {})


def transfer2_1(request):
    return render(request, 'app/transfer2_1.html', {})
